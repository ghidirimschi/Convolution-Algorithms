...
static void trivialDFT(int length, double complex *a, double complex omega, double complex *wsp) {
  double complex currOmega, tmpOmega = 1;
...
static void inplaceFFT(int length, double complex *x) {
...
static void inplaceInverseFFT(int length, double complex *x) {
  // note that length is expected to be a power of two!
  double complex *wsp = malloc(length*sizeof(double complex)); // workspace array
  double complex omega = cexp(2.0*PI*I/length);     // note that I is the c99 representation of the complex number 0 + i1
  inplaceCooleyTukeyFFT(length, x, omega, wsp);
...
void fftConvolutionNoPad(int lenH, int *h, int lenX, int *x, int *y) {
